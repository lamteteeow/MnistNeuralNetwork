...
public:
  ...
  double train(const Tensor &input_tensor, const Tensor &label_tensor)
  {
    ...
    error_tensor = relu1->backward(error_tensor);
  ...
  void evaluate(const Tensor &test_images, const Tensor &test_labels, unsigned int batch_size, const std::string &log_file)
  {
...

    {
      ...
      Tensor batch_images = test_images.middleRows(batch_start, std::min(batch_size, (unsigned int)test_images.rows() - batch_start));
      Tensor batch_labels = test_labels.middleRows(batch_start, std::min(batch_size, (unsigned int)test_labels.rows() - batch_start));
      ...
      for (int i = 0; i < predictions.rows(); i++)
      {
        ...
        log_stream << " - image " << (batch_start + i) << ": Prediction=" << predicted_label << ". Label=" << actual_label << std::endl;
  ...
  void fit(const Tensor &train_images, const Tensor &train_labels, unsigned int num_epochs, unsigned int batch_size)
  {
    for (unsigned int epoch = 0; epoch < num_epochs; ++epoch)
    {
      ...
      {
        Tensor batch_images = train_images.middleRows(i, std::min(batch_size, (unsigned int)train_images.rows() - i));
        Tensor batch_labels = train_labels.middleRows(i, std::min(batch_size, (unsigned int)train_labels.rows() - i));
